...
def graph():
    ...
    samples = 2
    fig, axs = plt.subplots(samples, len(models) + 1, figsize=(2.5*len(models), 2.5*samples))
    ...
    for i, name in enumerate(models.keys()):
        ...
        axs[-1, i+1].set_xlabel(f"{letters[i]} {models[name]['label'].split(' ')[-1]}")
        ...
        for index in indices:
            ...
            axs[k, i+1].imshow(plt.cm.jet(uncertainty[0][0].detach()))
            # axs[k, i].imshow(~(preds.argmax(1).cpu() == labels.argmax(1).cpu())[0] * 255)
